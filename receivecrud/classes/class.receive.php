...
class Receive{
	...
	public function new_receive_item($recid,$productid,$qty){
		/* Setting Timezone for DB */
		$NOW = new DateTime('now', new DateTimeZone('Asia/Manila'));
		$NOW = $NOW->format('Y-m-d H:i:s');
		$data = [
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_receive_items(rec_id, product_id, rec_qty) VALUES (?,?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row)
			{
	...
	public function save_to_inventory($id){
		...
		$stmt = $this->conn->prepare("INSERT INTO tbl_inventory(product_id, product_qty) VALUES (?,?)");
		try {
			$this->conn->beginTransaction();
			foreach ($data as $row){
				extract($row);
				$stmt->execute(array($product_id,$rec_qty));
			}
			//$id= $this->conn->lastInsertId();
			$this->conn->commit();
		}catch (Exception $e){
